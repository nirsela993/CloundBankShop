...
<body>
	...
	<body>
		...
		<script >
			// var paging = 0
			var bookTitleSearch = window.location.search.split("=")[1]
			window.loadBooks = ()=>{
				bookStoreAPI.getBooksByTitle(bookTitleSearch,(bookIds)=>{
					console.log('here',bookIds)
				// bookStoreAPI.getBookByTitle(bookTitleSearch,paging++,(books)=>{
					window.loadBooks = books = []
					for (var i = bookIds.length - 1; i >= 0; i--) {
						bookStoreAPI.getBookById(bookIds[i].id,(book)=>{
							console.log('recived',book)
							books.push(book)
